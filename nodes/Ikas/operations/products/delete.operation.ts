...
export async function deleteProducts(this: IExecuteFunctions, itemIndex: number): Promise<any> {
	try {
		...
		if (productIds.length === 0) {
			throw new NodeOperationError(this.getNode(), 'No valid product IDs provided for deletion', {
				itemIndex,
			});
		}
		...
		if (!success) {
			throw new NodeOperationError(this.getNode(), 'Failed to delete products', { itemIndex });
		}
