...
/**
 * Builds the customer update input object
 */
function buildUpdateCustomerInput(
...
export async function updateCustomer(
	...
	try {
		...
		const customerInput = buildUpdateCustomerInput(this, itemIndex);

		// Process additional fields
		processAdditionalFields(this, itemIndex, customerInput);

		// No strict validation for update, but ensure id is present
		if (!customerInput.id) {
			throw new NodeOperationError(this.getNode(), 'Customer ID is required for update', {
				itemIndex,
			});
		}
		...
		this.logger.info(JSON.stringify(response, null, 2), {
			message: 'Customer update response',
		});
		...
		throw new NodeOperationError(
			this.getNode(),
			`Failed to update customer: ${msg}${status ? ' | Status: ' + status : ''}${details ? ' | Details: ' + details : ''}`,
			{ itemIndex },
		);
	}
}
