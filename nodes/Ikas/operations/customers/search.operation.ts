...
export async function searchCustomers(this: IExecuteFunctions, itemIndex: number): Promise<IDataObject> {
  ...
  if (returnAll) {
    ...
    while (true) {
      ...
      const list = response.data?.listCustomer || { data: [], page, limit: pageLimit, count: 0 };
      const batch = (list.data as IDataObject[]) || [];
      collected = collected.concat(batch);
      const total = (list.count as number) ?? 0;
      if (collected.length >= total || batch.length < pageLimit) break;
      page += 1;
    }
  return { results: collected, paging: { page: 1, limit: collected.length, count: collected.length } } as IDataObject;
  }

  // single page
  const response = await ikasGraphQLRequest.call(this, GetCustomersQuery, vars);
  const list = response.data?.listCustomer || { data: [], page: 1, limit: 50, count: 0 };
  return { results: list.data, paging: { page: list.page, limit: list.limit, count: list.count } } as IDataObject;
}
